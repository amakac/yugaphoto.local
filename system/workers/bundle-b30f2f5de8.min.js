/* system/workers/edit.js */
var yellow={onLoad:function(e){yellow.edit.load(e)},onKeydown:function(e){yellow.edit.keydown(e)},onDrag:function(e){yellow.edit.drag(e)},onDrop:function(e){yellow.edit.drop(e)},onClick:function(e){yellow.edit.click(e)},onClickAction:function(e){yellow.edit.clickAction(e)},onPageShow:function(e){yellow.edit.pageShow(e)},onUpdatePane:function(){yellow.edit.updatePane(yellow.edit.paneId,yellow.edit.paneAction,yellow.edit.paneStatus)},onResizePane:function(){yellow.edit.resizePane(yellow.edit.paneId,yellow.edit.paneAction,yellow.edit.paneStatus)},action:function(action,status,arguments){yellow.edit.processAction(action,status,arguments)}};yellow.edit={paneId:0,paneAction:0,paneStatus:0,popupId:0,intervalId:0,load:function(e){var body=document.getElementsByTagName("body")[0];if(body&&body.firstChild&&!document.getElementById("yellow-bar")){this.createBar("yellow-bar");this.processAction(yellow.page.action,yellow.page.status);clearInterval(this.intervalId)}
...
this.processAction(element.getAttribute("data-action"),element.getAttribute("data-status"),element.getAttribute("data-arguments"))},pageShow:function(e){if(e.persisted&&yellow.user.email&&!this.getCookie("csrftoken")){window.location.reload()}},createBar:function(barId){var elementBar=document.createElement("div");elementBar.className="yellow-bar";elementBar.setAttribute("id",barId);if(barId=="yellow-bar"){yellow.toolbox.addEvent(document,"click",yellow.onClick);yellow.toolbox.addEvent(document,"keydown",yellow.onKeydown);yellow.toolbox.addEvent(window,"pageshow",yellow.onPageShow);yellow.toolbox.addEvent(window,"resize",yellow.onResizePane)}
var elementDiv=document.createElement("div");elementDiv.setAttribute("id",barId+"-content");if(yellow.user.name){elementDiv.innerHTML="<div class=\"yellow-bar-left\">"+this.getRawDataPaneAction("edit")+"</div>"+"<div class=\"yellow-bar-right\">"+this.getRawDataPaneAction("create")+this.getRawDataPaneAction("delete")+this.getRawDataPaneAction("menu",yellow.user.name,!0)+"</div>"+"<div class=\"yellow-bar-banner\"></div>"}else{elementDiv.innerHTML="&nbsp;"}
...
if(paneStatus=="none"){document.getElementById("yellow-pane-account-status").innerHTML=this.getText("AccountStatusNone");document.getElementById("yellow-pane-account-name").value=yellow.user.name;document.getElementById("yellow-pane-account-email").value=yellow.user.email;document.getElementById("yellow-pane-account-password").value="";if(document.getElementById("yellow-pane-account-"+yellow.user.language)){document.getElementById("yellow-pane-account-"+yellow.user.language).checked=!0}}
break;case "yellow-pane-configure":if(paneStatus=="none"){document.getElementById("yellow-pane-configure-status").innerHTML=this.getText("ConfigureStatusNone");document.getElementById("yellow-pane-configure-sitename").value=yellow.system.sitename;document.getElementById("yellow-pane-configure-author").value=yellow.system.author;document.getElementById("yellow-pane-configure-email").value=yellow.system.email}
...
if(paneStatus=="updates"){document.getElementById(paneId+"-submit").innerHTML=this.getText("UpdateButton");document.getElementById(paneId+"-submit").setAttribute("data-action","submit");document.getElementById(paneId+"-submit").setAttribute("data-arguments","action:update")}
...
var labels=yellow.language.editKeyboardLabels.split(/\s*,\s*/);if(navigator.platform.indexOf("Mac")==-1){shortcut=shortcut.toUpperCase().replace("CTRL+",labels[0]).replace("ALT+",labels[1]).replace("SHIFT+",labels[2])}else{shortcut=shortcut.toUpperCase().replace("CTRL+ALT+","ALT+CTRL+").replace("CTRL+SHIFT+","SHIFT+CTRL+");shortcut=shortcut.replace("CTRL+",labels[3]).replace("ALT+",labels[4]).replace("SHIFT+",labels[5])}
return shortcut},getText:function(key,prefix,postfix){if(!prefix)prefix="edit";if(!postfix)postfix="";key=prefix+yellow.toolbox.toUpperFirst(key)+yellow.toolbox.toUpperFirst(postfix);return(key in yellow.language)?yellow.language[key]:"["+key+"]"},getCookie:function(key){return yellow.toolbox.getCookie(key)},isUserAccess:function(action,location){var tokens=yellow.user.access.split(/\s*,\s*/);return tokens.indexOf(action)!=-1&&(!location||location.substring(0,yellow.user.home.length)==yellow.user.home)},isExpandable:function(name){return(name=="format"||name=="heading"||name=="list"||name=="emoji"||name=="icon")},isExtension:function(name){return name in yellow.system.coreExtensions}};yellow.editor={setMarkdown:function(element,prefix,type,toggle,callback){var information=this.getMarkdownInformation(element,prefix,type);var selectionStart=(information.type.indexOf("block")!=-1)?information.top:information.start;var selectionEnd=(information.type.indexOf("block")!=-1)?information.bottom:information.end;if(information.found&&toggle)information.type=information.type.replace("insert","remove");if(information.type=="remove-fenced-block"||information.type=="remove-inline"){selectionStart-=information.prefix.length;selectionEnd+=information.prefix.length}
...
if(yellow.system.coreDebugMode)console.log("yellow.editor.setMarkdown type:"+information.type);},getMarkdownInformation:function(element,prefix,type){var text=element.value;var start=element.selectionStart;var end=element.selectionEnd;var top=start,bottom=end;while(text.charAt(top-1)!="\n"&&top>0)top--;if(bottom==top&&bottom<text.length)bottom++;while(text.charAt(bottom-1)!="\n"&&bottom<text.length)bottom++;if(type=="insert-autodetect"){if(text.substring(start,end).indexOf("\n")!=-1){type="insert-fenced-block";prefix="```\n"}else{type="insert-inline";prefix="`"}}
